...
class ConfiguratorTestsHelper {
	...
	List<QueryResult> mockQueryResults() {
	...
	TabularResult createHighDimTabularResult(Map params) {
		...
		def rowsIterator = labelToData.collect { String label, List<Number> data ->
	...
	void createDataTypeResourceMock(Map<String, TabularResult<Double, AssayColumn>> highDimResults) {
		...
		HighDimensionDataTypeResource dataTypeResourceMock = mock(HighDimensionDataTypeResource)

		def termConstraintToResult = [:]

		highDimensionResourceMock.getSubResourceForType(DATA_TYPE_NAME_HIGH_DIMENSION).returns(dataTypeResourceMock)

		def projection = mock(Projection)
		dataTypeResourceMock.createProjection([:], Projection.DEFAULT_REAL_PROJECTION).returns(projection)
		...
		highDimResults.each { String conceptPath, TabularResult result ->
			...
			dataTypeResourceMock.createAssayConstraint(AssayConstraint.ONTOLOGY_TERM_CONSTRAINT,
					concept_key: Hacks.createConceptKeyFrom(conceptPath)).returns(mockConstraint)
		}
		...
		termConstraintToResult.each { AssayConstraint ontologyTermConstraint, TabularResult result ->
	...
	List<AssayColumn> createSampleAssays(int n) {
		(1..n).collect {
			createMockAssay("patient_${it}_subject_id", "sample_code_${it}")
		}
	}

	DataRow createRowForAssays(List<AssayColumn> assays, List<Double> data, String label) {
		createMockRow(
				dot(assays, data, { a, b -> [a, b] }).collectEntries(Closure.IDENTITY),
				label)
	}
	...
	List<String> createPatientRowLabels(int n) {
		(1..n).collect { 'patient #$' + it }
	}

	private static NULL_PLACEHOLDER = new Object()

	List<PatientRow> createPatientRows(int n, List<ClinicalVariableColumn> columns,
	                                   List<String> values, boolean relaxed = false) {
		def builder = ImmutableTable.builder()
		int i = 0

		def patients = createPatients n
		for (patient in patients) {
			for (columnVariable in columns) {
				if (values[i] != null) {
	...
	List<PatientRow> createPatientRows(List<Patient> patients, List<String> labels,
	                                   Table<Patient, ClinicalVariableColumn, Object> data,
	                                   boolean relaxed) {
	...
	void setupClinicalResult(int nPatients, List<ClinicalVariableColumn> columns,
	                         List valuesForColumns) {
		assert nPatients * columns.size() == valuesForColumns.size()

		TabularResult<ClinicalVariableColumn, PatientRow> clinicalResult = mock(TabularResult)
