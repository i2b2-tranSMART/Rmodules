...
class CensorColumnConfiguratorTests extends AbstractJobsIntegrationTest {
	...
	void testCensoring() {
		...
		setupClinicalResult(2, columns, ['var 1', null, 'var 3', null, null, null])
		...
		configuratorTestsHelper.play {
			...
			assertThat res,
					containsInAnyOrder(
							contains('subject id #1', CensorColumn.CENSORING_TRUE),
							contains('subject id #2', CensorColumn.CENSORING_FALSE)
					)
		}
	}
	...
	void testCensoringUnspecified() {
		...
		configuratorTestsHelper.play {
			...
			Column stubColumn = new StubColumn(header: 'STUB', data: ['subject id #1': 23, 'subject id #2': 42])
			...
			assertThat res,
					containsInAnyOrder(
							contains('subject id #1', 23, CensorColumn.CENSORING_FALSE),
							contains('subject id #2', 42, CensorColumn.CENSORING_FALSE)
					)
		}
	}
}
