...
import static jobs.steps.helpers.ConfiguratorTestsHelper.CONCEPT_PATH_HIGH_DIMENSION
import static jobs.steps.helpers.ConfiguratorTestsHelper.DATA_TYPE_NAME_CLINICAL
import static jobs.steps.helpers.ConfiguratorTestsHelper.DATA_TYPE_NAME_HIGH_DIMENSION
...
import static jobs.steps.helpers.ConfiguratorTestsHelper.SEARCH_KEYWORD_ID
...
class ContextNumericVariableColumnConfiguratorTests extends AbstractJobsIntegrationTest {
	...
	void testMultiHighDimensional() {
		...
		TabularResult result1 = createHighDimTabularResult(assays: assays1, data: data.result1)
		TabularResult result2 = createHighDimTabularResult(assays: assays2[1..2], data: data.result2)
	...
	void testSingleHighDimensional() {
		...
		TabularResult result = createHighDimTabularResult(assays: assays, data: data)
	...
	void testSingleClinical() {
		...
		play {
			...
			// no maps involved here
			assertThat Lists.newArrayList(table.result), contains(contains([('\\variable\\'): 34.0]))
		}
	}
}
