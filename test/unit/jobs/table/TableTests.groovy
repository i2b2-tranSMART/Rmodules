...
class TableTests {
	...
	void testSimpleCase() {
		...
		Column column = mockColumn(DATA_SOURCE_NAME,
				twoRowDataSource,
				twoRows,
		...
		play {
			...
			assertThat result, contains(sampleValues.collect { is([it]) })
		}
	}
	...
	void testTwoColumnsSameDataSource() {
		...
		List<Column> columns = sampleValues.collect { values ->
			mockColumn(DATA_SOURCE_NAME, twoRowDataSource, twoRows, combinePKsAndValues(twoPKs, values))
		}
		...
		play {
			...
			testee.buildTable()
			assertThat testee.result, contains(sampleValues.transpose().collect { is it })
		}
	}
	...
	void testMissingData() {
		...
		List<Column> columns = sampleValues.collect { columnValues ->
			mockColumn(DATA_SOURCE_NAME, twoRowDataSource, twoRows, combinePKsAndValues(twoPKs, columnValues))
		}
		...
		play {
			...
			assertThat testee.result, contains(
					sampleValues.transpose().collect {
						is it.collect { it ?: replacementString }
					})
		}
	}
	...
	void testTwoDataSourcesOnePerColumn() {
		...
		play {
			...
			testee.buildTable()
			assertThat testee.result, contains(is(['datasource 1', 'datasource 2']))
		}
	}
	...
	void testNullReplacementsRemoveLine() {
		...
		play {
			...
			assertThat testee.result, contains(is(['col 1, row 2', 'col 2, row 2']))
	...
	void testOnAllDataSourcesDepleted() {
		List<Column> columns = []
		List values = ['row1', 'row2']

		columns << new PrimaryKeyColumn(header: 'PK')
		columns << mockColumn(DATA_SOURCE_NAME, twoRowDataSource, twoRows, combinePKsAndValues(twoPKs, values))
	...
	private @Lazy
	List twoRows = [mock(Object), mock(Object)]

	private @Lazy
	List<String> twoPKs = ['rowPK1', 'rowPK2']

	private @Lazy
	Iterable twoRowDataSource = mockDataSource(twoRows)

	private Column mockColumn(String dataSourceName, Iterable dataSource,
	                          List rows, List<Map> returnValues,
	                          Closure valueTransformer = null) {
